...
interface ArchitectsFormProps {
    data: any;
    onChange: (data: any) => void;
    readOnly?: boolean;
}

const ArchitectsForm = ({ data, onChange, readOnly }: ArchitectsFormProps) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-2">
            ...
            <div className="space-y-1.5 md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1.5">
                    <Label className="text-slate-700 font-semibold">Firm / Individual Name</Label>
                    <Input value={data.firmName || ''} onChange={e => handleChange('firmName', e.target.value)} placeholder="Entry firm name" disabled={readOnly} />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                    <Label className="text-slate-700 font-semibold">Authorized Person Name</Label>
            ...
            <div className="space-y-1.5 md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="space-y-1.5">
                    <Label className="text-slate-700 font-semibold">Contact Details</Label>
                    <Input value={data.contactNumber || ''} onChange={e => handleChange('contactNumber', e.target.value)} placeholder="Phone/Email" disabled={readOnly} />
                </div>
                <div className="space-y-1.5">
                    <Label className="text-slate-700 font-semibold">Purchase Person Name</Label>
                    <Input value={data.purchasePersonName || ''} onChange={e => handleChange('purchasePersonName', e.target.value)} placeholder="Name" disabled={readOnly} />
                </div>
                <div className="space-y-1.5">
                    <Label className="text-slate-700 font-semibold">Purchase Person Contact Number</Label>
                    <Input value={data.purchasePersonContact || ''} onChange={e => handleChange('purchasePersonContact', e.target.value)} placeholder="Phone" disabled={readOnly} />
                </div>
            </div>
